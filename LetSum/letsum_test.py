...
def tokennize_file(file):
	'''
	Return the content of the file in a list. Every index contains a sentence.
	:param file: file path and name to tokenize
	:return: List with a sentence in a index
	'''
	with open(file, 'r') as f:
		txt = f.read()
	tokenized = nltk.tokenize.sent_tokenize(txt, language='french')
...
def LetSum(file):
	'''
	receives a txt file, produce categories for each of them and returns a summary
	:param file: file path to summarize
	:return: str of the summary produced
	'''
	label = OrderedDict()
	scores = OrderedDict()
	lines = {}

	t = tokennize_file(file)
	...
	print('category_txt: ',category_txt)
